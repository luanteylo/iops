...
**IOR** can be used to test the **performance** of parallel storage systems using various interfaces and access patterns. When different processes write to different files, it's called **N-N**. When writing to the same file, it is called **N-1 "strategy"**, and so it can be in "any" order, a **sequential** order could be like process 1 write to the first part of the file and process 2 write to the second, and so on. On the other hand a **random** order is random writing. So those **IO patterns** also affect the **performance** of **PFS**. 
...
## Task 3
### TestIOR class
`get\_ior\_command` represents the IOR command line for example 

 and `generate\_batch\_file` Receives the fullPath and generates a batch file capable of executing the IOR command generated by the previous method.
 
**DATE**: ven. 12 avril 2024 09:43:03
Contuning the task 3.

**DATE**: lun. 15 avril 2024 10:05:16 

To day I continue Task 3 and push the code when finished this task. Afternoon, I would be to look at task 4. This week my tutor is in Japon for conference. I only I discuss with him by messaging or video call. 

task 3 almost finish, the method `get\_ior\_command` is the retourning the ior commannd to be pass to evaluate a file distribution system. It is done.
And `generate\_batch\_file` is generating the parameters, those parameters that should be in sbatch file. So just setting parameters in the TestIOR and pass as argument to `slurm_script` method in order to launch the command.


## Task 4

The Test execution procedures. Fow now, only slurm job manager supported. 
Here, I implement a local class in a `submitter` file to execute the benchmark without a job manager. Fow now the `submit_local` does just `subprocess.run` command and `print`. But handling if the Test is finish and run new one for example.




**questions**
How does look a local command running the test ?

How is the strategy to submit locally ?

Does we need, I local template like the one for slurm ? Yes

Does we need to generate a local_script ? Yes

**DATE**: mar. 16 avril 2024 09:45:31 
I continue this task, there is modification that Luan reported to me. One of to complete the `local_script` function to generate a script for local execution. Therefore one answer to the above question, I need a local template file.

And also, I did not testing plafrim execution since yesterday I worked local.
Now I think, I change as expected, I push and retrieve on plafrim from the reposotory and change for slurm execution.

**DATE**: mer. 17 avril 2024 09:25:29 

Before starting the today tasks, I push yesterday generate batch file by adapting the parameters that has a if-statement in the `slurm_template` in this case only passing if the condition is true.

Now I do merge request to ask Luan for review and merge to development branch (dev-1.0) if valid. But I found yesterday I cannt do a merge request because there is open one for this merge. So I may ask luan to close it or I do. 
...
task 3 almost finish, the method `get\_ior\_command` is the retourning the ior commannd to be pass to evaluate a file distribution system. It is done.
And `generate\_batch\_file` is generating the parameters, those parameters that should be in sbatch file. So just setting parameters in the TestIOR and the IOR command and pass as argument to `slurm_script` method in order to launch the command.
