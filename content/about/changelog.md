...
- `benchmark.timestamp_precision` (`seconds`, the previous and current default, or `milliseconds`) sets the resolution of the recorded execution timestamps: `metadata.submission_time`, `metadata.job_start`, and `metadata.end` in the results, and the matching fields in `__iops_status.json`. Whole seconds are too coarse to tell short local executions apart, and the timestamps were previously formatted with `time.strftime`, which cannot express a fractional part at all. With `milliseconds` they are recorded as `2026-07-27 14:26:04.123`, truncated rather than rounded so a timestamp never overshoots the instant it records. Note that under SLURM `job_start` is recorded when polling observes the job leave the queue, so its real resolution remains `slurm_options.poll_interval`. Changing the setting does not invalidate cached results; a run reusing a cache written under the other precision produces a results set holding both formats, which reports read correctly.
