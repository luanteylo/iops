...
- `benchmark.probes.io_sampling` (default `false`) samples I/O activity during execution. It reads two counter sources every `sampling_interval`, because neither one alone describes both a local disk and a network filesystem: `/proc/diskstats` for block devices, which reports nothing for NFS since that traffic never reaches a client block device, and `/proc/self/mountstats` for NFS mounts, which reports nothing for local disks. Each row of `__iops_io_trace_<hostname>_<attempt_id>.csv` is tagged with its source, so a study comparing storage backends gets both arms rather than zeros on one side. The run summary gains `io_read_gb`, `io_write_gb`, their per-source counterparts (`io_disk_*`, `io_nfs_*`), average and peak throughput, and average operations per second; all of them are registered as report metrics and appear in the Resource Sampling table. NFS byte counts come from the server-read and server-written counters, so reads the page cache satisfied locally are excluded and the two backends are compared like with like. Block devices are counted once: partitions and virtual devices (`dm-*`, `md*`, `loop*`) are excluded to avoid double counting. Counters are node-level, so they include anything else running on the node.
- `benchmark.probes.io_paths` names the storage I/O sampling measures, and is required whenever `io_sampling` is enabled, e.g. `["{{ execution_dir }}", "/scratch/input"]`. There is no "measure everything" mode on purpose: a node usually has storage the benchmark never touches, so a run writing to `/tmp` on a node that also mounts NFS would report both and neither number would describe the run. Paths are Jinja2 templates rendered per execution, so they can name a directory the run creates, and they are resolved on the compute node because mount tables differ across an allocation. A path that does not exist yet resolves through its nearest existing ancestor. A local path resolves to the whole block device behind it (a partition to its parent disk, an LVM or md volume to the disks underneath), so block scoping is only as precise as the device layout; an NFS path resolves to that mount exactly. A path with no countable storage behind it, such as anything on tmpfs, is reported rather than silently zero: the run logs a warning and every node writes `__iops_io_targets_<hostname>_<attempt_id>.json` recording what each path resolved to. Throughput is reported two ways, because sampling covers the whole script and most benchmarks spend much of it not doing I/O: `io_*_mbs_avg` divides by the full traced window and answers what the job moved per second of runtime, while `io_*_mbs_active` divides by `io_*_active_s`, the time that direction actually moved data, and answers how fast the storage went while in use. Volumes and peaks are unaffected by idle samples either way. Trace rows gain a `path` column naming the configured entry they came from. Paths are a filter rather than a per-path breakdown: counters are deduplicated, so two directories on the same disk are counted once and labelled with both, since the kernel keeps no per-directory counters to tell them apart.

- Escalating variables (`escalate`), a new variable kind that pairs with an adaptive variable to perform a staircase search. A plain adaptive probe finishes the first time `stop_when` triggers; with an escalating variable, the escalating value advances and the same adaptive value is retested instead, and the probe ends only when the escalation values run out. This expresses searches where one variable should advance precisely when the other stalls, such as finding the smallest `number_of_blocks` each `problem_size` needs, equivalently the largest `problem_size` each `number_of_blocks` can handle. The result is reported as a frontier, one rung per escalation value, in the run summary and under `adaptive_results` in `__iops_run_metadata.json`, with the new `stop_reason` value `escalation_exhausted`. Only one axis moves per step, so the search stays well defined; the cost is `P + B - 1` runs against up to `P x B` for sweeping the second axis. At most one escalating variable per config, and it requires an adaptive variable to extend. The staircase never de-escalates, so it assumes a value that fails at one adaptive value also fails at every later one. See [Adaptive Variables](/iops/user-guide/adaptive-variables/#staircase-search-with-an-escalating-variable).

- HTML reports now show a staircase search as a frontier. The Adaptive Probing Results section previously reported only the probe's final state, a single `stop_value`/`last_value_before_stop` pair, which for a staircase is the point where the search ran out of escalation values and hides the result entirely. The section now leads with a **Search Frontier** table, one row per escalation value with the furthest adaptive value it reached, and a **Search Path** plot showing every test the search ran (successes as green circles, stop-condition hits as red crosses) with a step line tracing the frontier. The escalation axis is spaced by position, so values like 1, 4, 16 stay evenly spaced. The Probing Configuration block lists the escalating variable and its values, and the section text describes the staircase rather than a single-threshold probe. Plain adaptive runs are unchanged.

- A `gallery` extra installs Pillow, which `reporting.gallery.max_width` needs to downscale images: `pip install iops-benchmark[gallery]`. Pillow was documented as a requirement of that option but was not declared anywhere, so it had to be installed by hand.
...
- I/O sampling scoped with `io_paths` no longer reports NFS mounts the paths never resolved to. The NFS filter read an empty selection as "no filtering", so a run scoped to a local directory silently counted every NFS mount on the node. On a cluster whose nodes mount a package store and NFS home directories, a run scoped to `/tmp` attributed nearly all of its reported reads to module loading rather than to the benchmark. Once paths are configured, an empty selection for a source now correctly means nothing from that source.
- `__iops_resource_summary.csv` is no longer lost when executions report different metric sets. The header was taken from the first row with data, so any later row carrying a column that row lacked failed to write and the whole summary was dropped with a `dict contains fields not in fieldnames` warning. This happened when a GPU reported a field as unavailable in one execution but not another, and when an execution finished too quickly for its samplers to collect anything and so reported only the sample counters. Columns are now the union across all rows, in first-seen order, and cells with no value are left empty.
- Resource and GPU sampling now cover every node of a multi-node job on OAR and PBS, not just the head node. The samplers fanned out only when `SLURM_JOB_ID` was set and `SLURM_NNODES > 1`, using `srun`; on any other scheduler that test failed and the sampler ran in the background of the head node alone, so a 4-node run reported `nodes_traced: 1` and its memory, CPU, and imbalance metrics described a single node. A shared node launcher (`__iops_node_launcher.sh`) now resolves the node list per scheduler (`scontrol show hostnames` for SLURM, `$OAR_NODEFILE` for OAR, `$PBS_NODEFILE` for PBS) and starts one sampler per node (`srun --overlap`, `oarsh`, or `ssh` respectively), falling back to the local node when no scheduler is detected. The per-attempt id used in sentinel and trace filenames now also recognises `OAR_JOB_ID` and `PBS_JOBID`, and is forwarded to remote nodes so all of them agree on the sentinel path. Remote traces require the execution directory to be on a shared filesystem; `nodes_traced` in `__iops_resource_summary.csv` reports how many nodes actually returned samples.
- `iops report` now works for adaptive runs that have no swept variables. Report variables defaulted to numeric *swept* variables only, and an adaptive variable is never swept, so a config whose only varying parameter was the adaptive one failed with `No report variables found. Either specify report_vars in config or ensure you have numeric swept variables.` The default now includes the variables the search itself drives (adaptive and escalating), which in such a run is the only axis there is to plot against. An explicit `report_vars` still takes precedence, and string-typed variables are still excluded. Runs recorded by earlier versions report correctly without regeneration.
...
- Watch mode keypresses (`q`, `p`, scrolling) no longer become unresponsive on large runs. The render loop serviced data refreshes before the keyboard, so once a frame took longer than the refresh interval the view redrew continuously without ever polling for input. Input is now drained first, every frame.
