...
- `__iops_resource_summary.csv` is no longer lost when the first execution produced no usable samples. The header was taken from the first row with data, so a short execution that reported only the sample counters made every later row fail to write and the summary was dropped with a `dict contains fields not in fieldnames` warning. Columns are now the union across all rows, and cells with no value are left empty.
