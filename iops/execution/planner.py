...
from iops.config.models import GenericBenchmarkConfig, MPIConfig
...
class BasePlanner(ABC, HasLogger):
    ...
    def _wrap_script_with_mpi(self, script_text: str, test: ExecutionInstance) -> str:
        """
        Wrap script with MPI launch logic if mpi config is present.

        For single-allocation mode, this wraps the user's script to:
        1. Only run MPI commands on node 0 (via SLURM_NODEID check)
        2. Build the nodelist from SLURM_JOB_NODELIST
        3. Launch MPI with proper flags

        The script structure becomes:
        - Shebang and SBATCH directives (preserved)
        - IOPS injection block (if present)
        - Setup commands (module load, exports, etc.)
        - MPI launch block (SLURM_NODEID=0 check + mpirun)

        Args:
            script_text: Original script content (may already have IOPS injection)
            test: ExecutionInstance with mpi_config and vars

        Returns:
            Script text wrapped with MPI launch logic
        """
        if not test.mpi_config:
            return script_text

        mpi = test.mpi_config

        # Resolve nodes and ppn values using the test's render context
        ctx = test._render_context()
        nodes_value = self._resolve_mpi_value(mpi.nodes, ctx)
        ppn_value = self._resolve_mpi_value(mpi.ppn, ctx)

        if ppn_value is None:
            self.logger.error("MPI ppn value could not be resolved")
            return script_text

        # Calculate total np
        if nodes_value is not None:
            np_value = nodes_value * ppn_value
        else:
            # "all" - we'll compute at runtime
            np_value = None

        # Generate the MPI launch wrapper
        return self._generate_mpi_wrapper(script_text, nodes_value, ppn_value, np_value, mpi, test)

    def _resolve_mpi_value(self, value: Optional[str], ctx: dict) -> Optional[int]:
        """
        Resolve a value that might be a Jinja2 template.

        Args:
            value: String value - "all", "{{ var }}", or a number
            ctx: Render context with variable values

        Returns:
            Resolved integer value, or None for "all"
        """
        if value is None:
            return None
        ...
        if value == "all":
            return None  # Special case: use full allocation

        if "{{" in value:
            # Jinja2 template - render it
            try:
                rendered = _render_template(value, ctx)
                return int(rendered)
            except (ValueError, TypeError) as e:
                self.logger.warning(f"Could not resolve MPI value '{value}': {e}")
                return None
        ...
        try:
            ...
            self.logger.warning(f"Invalid MPI value '{value}': not a valid integer")
            return None

    def _generate_mpi_wrapper(
        self,
        script_text: str,
        nodes_value: Optional[int],
        ppn_value: int,
        ...
        """
        Generate the MPI launch wrapper around the user's script.

        The wrapper structure:
        1. Shebang + SBATCH directives (preserved)
        2. IOPS injection (preserved if present)
        3. if SLURM_NODEID=0 block containing:
           - User's setup commands (module load, exports, etc.)
           - NODELIST construction
           - MPI launch command replacing the rendered command
        4. Non-node-0 nodes exit immediately (implicit, nothing after the if block)

        Args:
            script_text: Original script content (already rendered)
            nodes_value: Number of nodes (None = all)
            ppn_value: Processes per node
            np_value: Total MPI processes (None = compute at runtime)
            mpi: MPI configuration
            test: ExecutionInstance for command rendering

        Returns:
            Wrapped script text
        """
        lines = script_text.split('\n')
        rendered_command = test.command.strip()

        # Find the end of the header section (shebang + SBATCH + IOPS injection)
        header_end = 0
        ...
        # Skip SBATCH directives and comments until actual commands
        while header_end < len(lines):
            line = lines[header_end]
            stripped = line.strip()
            if stripped == '':  # Blank line - continue
                header_end += 1
            elif stripped.startswith('#'):  # Comment or #SBATCH - continue
                header_end += 1
            else:  # Actual command - check if it's IOPS injection
                if stripped.startswith('source ') and '__iops_' in stripped:
                    # Part of IOPS injection block - continue
                    header_end += 1
                elif stripped.startswith('# ===== IOPS'):
                    # IOPS injection marker - continue
                    header_end += 1
                else:
                    # Real user command - stop here
                    break

        # Split script into header and body
        header_lines = lines[:header_end]
        body_lines = lines[header_end:]

        # Build the MPI wrapper
        wrapper_lines = []

        # Start with the SLURM_NODEID check
        wrapper_lines.append('')
        wrapper_lines.append('# ===== MPI LAUNCH WRAPPER (generated by IOPS) =====')
        ...
        # Include user's body (setup commands, etc.) inside the if block
        # Replace the rendered command with the MPI launch
        for body_line in body_lines:
            ...
            if rendered_command and stripped == rendered_command:
                ...
                wrapper_lines.append(f'  {new_line.lstrip()}')
                command_replaced = True
            else:
                # Indent the line (since it's now inside the if block)
                if stripped:  # Non-empty line
                    wrapper_lines.append(f'  {body_line}')
                else:
                    wrapper_lines.append(body_line)

        # If we didn't find the command to replace, append the MPI launch at the end
        ...
        wrapper_lines.append('')
        wrapper_lines.append('fi')
        wrapper_lines.append('# ===== END MPI LAUNCH WRAPPER =====')

        # Combine header and wrapper
        return '\n'.join(header_lines + wrapper_lines)

    def _generate_nodelist_command(self, nodes_value: Optional[int], ppn_value: int) -> str:
        """
        Generate bash command to construct the MPI nodelist.

        For mpirun with --host, we need a comma-separated list like:
        node1:ppn,node2:ppn,node3:ppn
    ...
    def _generate_mpirun_command(
        ...
        Flags used:
        - --host $__IOPS_NODELIST: Use our computed nodelist
        ...
        # Host specification
        parts.append("--host $__IOPS_NODELIST")

        # Process count
        if np_value is not None:
            parts.append(f"-np {np_value}")
        else:
            # Compute at runtime: nodes * ppn
            parts.append(f'-np $(echo "$__IOPS_NODELIST" | tr "," "\\n" | wc -l | xargs -I{{}} expr {{}} \\* {ppn_value})')

        # Process mapping
        parts.append(f"--map-by ppr:{ppn_value}:node")

        # Required flags for running within SLURM allocation
    ...
    def _generate_srun_command(
        ...
        """
        Generate srun command with all necessary flags.
        ...
        # Node count
        if nodes_value is not None:
            parts.append(f"--nodes={nodes_value}")

        # Tasks per node
        parts.append(f"--ntasks-per-node={ppn_value}")

        # Total tasks
        if np_value is not None:
            parts.append(f"--ntasks={np_value}")

        # Use overlap to allow multiple sruns within the allocation
