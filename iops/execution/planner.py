...
def _build_version_probe_script(versions: Dict[str, str], exec_dir: str) -> str:
    ...
    lines = [
        ...
        "    _iops_json_escape() {",
        "      # Escape backslash and double-quote, then the control characters a",
        "      # JSON string cannot carry raw. Bash parameter expansion rather than",
        "      # sed, which got this wrong on both platforms: the ':a;N;$!ba'",
        "      # newline-slurping idiom is GNU-only so BSD/macOS sed left raw",
        "      # newlines in, and even under GNU sed the tab substitution did not",
        "      # take, so any command whose output held a tab or a newline wrote an",
        "      # unparseable __iops_versions.json.",
        "      local _s",
        "      _s=$(cat)",
        r'      _s=${_s//\\/\\\\}',
        r'      _s=${_s//\"/\\\"}',
