...
# Execution timestamp formats, selected by benchmark.timestamp_precision.
# Both are accepted by datetime.fromisoformat, which is how the runner,
# planner, and watch mode read these values back.
...
class BaseExecutor(ABC, HasLogger):
    ...
    def _now(self) -> str:
        """
        Current local time formatted for the execution timestamp metadata
        (__submission_time, __job_start, __end).

        Resolution follows benchmark.timestamp_precision. Milliseconds are
        truncated from the microsecond field rather than rounded, so the
        timestamp never overshoots the instant it records.
        """
        if self.cfg.benchmark.timestamp_precision == "milliseconds":
            return datetime.now().strftime(TIMESTAMP_FORMAT_MS)[:-3]
...
class SlurmExecutor(BaseExecutor):
    ...
    def submit(self, test) -> None:
        ...
        try:
            test.metadata["__submission_time"] = self._now()
        ...
        while True:
            ...
            if state == "PENDING":
                ...
                if "__job_start" not in test.metadata:
                    test.metadata["__job_start"] = self._now()
