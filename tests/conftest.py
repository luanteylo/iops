...
@pytest.fixture
def sample_round_config_dict(tmp_workdir):
    """Create a sample multi-round configuration."""
    return {
        "benchmark": {
        ...
        "scripts": [
            {
                "name": "bandwidth_test",
                "submit": "bash",
                "script_template": "#!/bin/bash\necho 'bandwidth: 100' > {{ execution_dir }}/output.txt",
                "parser": {
                    "file": "{{ execution_dir }}/output.txt",
                    "metrics": [{"name": "bandwidth", "type": "float"}],
                    "parser_script": "def parse(file_path):\n    with open(file_path) as f:\n        line = f.read().strip()\n    return {'bandwidth': float(line.split(':')[1])}"
                }
            }
        ],
        "output": {
